...
\section{Overview of the thesis}
    ...
    In Chapter 2, first the telescopes and instruments that supplied the data analysed in this thesis are described.
    Second, the signal analysis methods used to determine the periods and phase difference were detailed.
    ...
    In Chapter 5, the focus shifted from analysing the cross-sectional of magnetic structures to the analysis of a Running Penumbral waves (RPWs) in a magnetic pore.
    RPWs have only observed in sunspot penumbras; however, this analysis details the presence of events that have the identical signature of RPWs emanating from an magnetic pore.
...
\section{Summary of results}
    ...
    In Chapter 2, after the description of both the sources of solar data and the signal analysis methods employed; there was a study on the principle method that is used to measure the cross-sectional area of the magnetic structures that were under investigation.
    The reason for this study was to understand if the overall method has any bias with regards to the choice of the sigma multiplier.
    The selected sigma multiplier is important as this results in the cross-sectional area and total intensity signals, which are then analysed to identify waves as as well used for magneto-seismology.
    As a result, any potential bias has to be understood.
    In order to undertake this study, high-quality ground-based data was used from the Dunn Solar Telescope (DST), using two instruments that are installed, the \textit{Rapid Oscillations in the Solar Atmosphere} (ROSA) and the \textit{Interferometric Bidimensional Spectrometer} (IBIS).
