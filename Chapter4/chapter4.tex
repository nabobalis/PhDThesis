...
\chapter[Slow MHD sausage waves]{Slow MHD sausage waves within small-scale photospheric magnetic structures}
\label{chapter4}
...
\label{Intro}
    ...
    While, this technique has been used for many years on the solar corona, only recently has it been applied to the lower solar atmosphere.
    ...
    Standing waves have been potentially seen in the lower solar atmosphere; using the Hinode space-borne instrument suit, \citet{PMHDW} observed pores and inter-granular magnetic structures, finding perturbations in the magnetic field, velocity and intensity.
    ...
    Section \ref{res} contains the results of the data analysis while Section \ref{conc} summarises.  
...
\label{DnA}
    ...
    \begin{figure}
        ...
        \caption{The magnetic pores at the start of the observation sequence (\textit{Upper panels}).
    ...
    Note, that the DOT does not have a adaptive optics system.
    ...
    The standard Morlet wavelet, which is a plane sine wave with an amplitude modulated by a Gaussian function, was chosen due to its high resolution in the frequency domain.
...
%    The full derivation can be found in Appendix 1.
    ...
    This depends on the sign of $c_s^2k^2-\omega^2$, which we assume is positive.
    ...
    Further, as there are known difficulties with using the Stokes profiles, observing changes in pore area could provide a novel way of validating or refuting the obtained magnetic oscillations derived from Stokes profiles.
    ...
    \citet{luna-cardozo} modelled the effect on the ratio of the fundamental and first overtone periods for a vertical flux tube sandwiched between the photosphere and transition region.
    ...
    \begin{align}
        ...
        \label{den_strat}
    \end{align} 
    where $\kappa_{1}$ is the ratio of the density at the upper end to the density at the lower footpoint.
    ...
    Both Equations \ref{mag_strat} and \ref{den_strat} modelling the frequency ratio of standing oscillations indicate that the ratio of the first harmonic to the fundamental will always be less than two.
...
\label{res}
    ...
    \begin{sidewaysfigure}
        ...
        \end{subfigure}
        \caption{Evolution of the area of the pore (\textit{Upper panels}).
    ...
    This suggests, we are most likely observing the same oscillatory phenomena as these previous observations, but here we are now able to identify a very important characteristic of the detected oscillations: the angular mode number, $m$.
    ...
    This can be achieved using either a FFT power spectrum or the IMF's amplitude from the EMD analysis.
    ...
    Note, the increase in radius is around $100$ km meaning the perturbation is only of the order of 1 pixel (at the DOT's resolution).
    ...
    They are, however, the order of observed horizontal flows around pores.
    ...
    This needs to be investigated in the future, as the authors are unaware of which MHD mode would cause this behaviour. 
    ...
    These then, lead to the radial perturbation amplitude of 69.1, 74.2 and 117 km and the radial velocity perturbation as 12, 5.32 and 2.72 km s$^{-1}$, respectively.
    ...
    There is only one period that was found to have a non-subsonic velocity (2-3 minutes), while the rest are all sub-sonic.
    ...
    With the important inclination that the observed waves are trapped, there is a possibility of them being standing ones.
    ...
    For the first case; Equation \ref{den_strat} is solved with typical density values from the VAL-III C model \citet{1981ApJS...45..635V} at the apex (transition region) and footpoint (photosphere) of the flux tube.
    ...
    The resulting value for the period ratio in this circumstance is 1.44.
    Using the model given by \citet{Maltby1986}, which models a sunspot umbra, this period ratio is 1.38.
